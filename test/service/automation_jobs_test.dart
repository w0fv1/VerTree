...
import 'package:vertree/modules/automation/application/automation_jobs.dart';

void main() {
  test('cancels queued work and preserves results of completed work', () async {
    final jobs = AutomationJobs(AppEvents());
    var ran = false;
    final cancelled = jobs.start('test', (_) async {
      ran = true;
      return null;
    });
    ...
    expect(done.result, 'saved');
  });
  test('limits concurrent work', () async {
    final jobs = AutomationJobs(AppEvents());
    final finish = Completer<void>();
    for (var i = 0; i < 4; i++) {
      jobs.start('wait', (_) => finish.future);
    }
