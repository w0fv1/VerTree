...
class LocalHttpApiServer {
  ...
  final LocalHttpApiSecurity _security;
  ...
  String get accessToken => _security.accessToken;
  ...
  Future<void> start() async {
    ...
    for (
      ...
      try {
        ...
        _onLogInfo('Port $candidate unavailable for local HTTP API: $e');
      }
    }
  ...
  Future<void> stop() async {
    ...
    if (server != null) {
      ...
      _onLogInfo('Local HTTP API stopped');
    }
  }
  ...
  Future<void> _listen(HttpServer server) async {
    try {
      ...
      _onLogError('Local HTTP API listen loop failed: $e');
    }
  }
  ...
  Future<void> _handleRequest(HttpRequest request) async {
    ...
    try {
      ...
      final requestSecurity = _security.authorize(
        _requestContext(request),
        port: _port ?? request.connectionInfo?.localPort ?? defaultPort,
        requiresAuthentication: false,
        requiresJsonBody: false,
      );
      if (!requestSecurity.isAllowed) {
        await _writeSecurityFailure(request, requestSecurity, startedAt);
        return;
      }
  ...
  Future<void> _dispatch(
    ...
    for (final definition in _routes) {
      ...
      final requestSecurity = _security.authorize(
        ...
        requiresJsonBody: definition.requestBody != null,
      );
      if (!requestSecurity.isAllowed) {
        await _writeSecurityFailure(request, requestSecurity, startedAt);
        return;
  ...
  List<LocalHttpApiRoute> _buildRoutes() {
    return [
      ...
      LocalHttpApiRoute(
        method: 'GET',
        pathTemplate: '/ping',
        summary: 'Probe the local API',
        description:
            'Returns a minimal readiness response without local state.',
        tags: const ['system'],
        requiresAuthentication: false,
        handler: _handlePing,
      ),
  ...
  Future<void> _handlePing(
    ...
    await _writeSuccess(
      request,
      data: const {'service': 'vertree', 'status': 'ok'},
      startedAt: startedAt,
    );
  }
  ...
  LocalHttpApiRequestContext _requestContext(HttpRequest request) {
  ...
  Future<void> _writeSecurityFailure(
    HttpRequest request,
    LocalHttpApiAuthorizationResult result,
    DateTime startedAt,
  ) async {
    if (result.statusCode == HttpStatus.unauthorized) {
      request.response.headers.set(
        HttpHeaders.wwwAuthenticateHeader,
        'Bearer realm="vertree-local-api"',
      );
    }
    await _writeJson(
      request,
      statusCode: result.statusCode,
      body: _errorBody(request, result.code, result.message, startedAt),
    );
  }
