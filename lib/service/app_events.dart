...
class AppEvent {
  AppEvent(this.id, this.type, Map<String, dynamic> data)
...
class AppEvents {
  ...
  static final instance = AppEvents();
  final int capacity;
  int _sequence = 0;
  ...
  void emit(String type, Map<String, dynamic> data) {
    final event = AppEvent(++_sequence, type, data);
    _history.add(event);
    if (_history.length > capacity) _history.removeAt(0);
    _live.add(event);
  }

  Stream<AppEvent> watch({int? after}) {
    if (after != null &&
        (after > _sequence ||
            after < 0 ||
