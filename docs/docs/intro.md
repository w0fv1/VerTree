...
- 本地文件预览：通过 Office Viewer 预览办公文档、PDF、图片、音视频、电子书、邮件、压缩包等；支持幻灯片、PSD 和 XMind 可视化
- 快速预览：版本树节点、Windows 文件右键菜单和 `vertree preview <path>` 均可打开预览，连续打开时自动关闭上一个预览窗口
- 大文件预览：取消固定 64 MiB 上限，媒体、PDF 和 Parquet 可分段读取；其他格式仍受解析器和可用内存限制
- 手动备份与快速备份：保留阶段性成果
...
- 局域网权限引导：浏览器阻止本地网络访问时，分享页会明确提示先授权再重试，并限制失败探测时的候选地址膨胀
- 开发控制：可通过 `dev_server.py` 托管 `flutter run` 并执行 reload / hot restart
- 平台集成：
  - Windows：托盘、右键菜单、Win11 新菜单开关和新设备注册修复
